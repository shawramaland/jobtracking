...
function MockInterview() {
  ...
  const sel = { padding: "6px 10px", fontSize: "12px", background: "#111", border: "1px solid #222", color: "#e2e8f0", fontFamily: "inherit", borderRadius: "6px", cursor: "pointer", outline: "none" };
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "14px" }}>
        ...
        <button onClick={() => { setHideRated(s => !s); goTo(0); }}
          style={{ padding: "6px 12px", borderRadius: "6px", border: `1px solid ${hideRated ? "#e63946" : "#333"}`, background: "transparent", color: hideRated ? "#e63946" : "#555", fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", gap: "12px", marginBottom: "14px", fontSize: "11px", flexWrap: "wrap" }}>
        <span style={{ color: "#555" }}>{filtered.length} questions</span>
        {RATINGS.map(r => (
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {/* Card */}
          <div style={{ background: "#111", border: `1px solid ${currentRating ? currentRating.border : "#222"}`, borderRadius: "10px", padding: "20px", marginBottom: "12px", transition: "border-color 0.3s" }}>

            {/* Meta */}
            <div style={{ display: "flex", gap: "8px", marginBottom: "12px", flexWrap: "wrap", alignItems: "center" }}>
              <span style={{ fontSize: "10px", padding: "2px 8px", borderRadius: "4px", background: "#1a0a0a", color: "#e63946", border: "1px solid #e6394640" }}>{question.category}</span>
              <span style={{ fontSize: "10px", padding: "2px 8px", borderRadius: "4px", background: "#0d0d0d", color: DIFF_COLORS[question.difficulty], border: `1px solid ${DIFF_COLORS[question.difficulty]}40` }}>{question.difficulty}</span>
              {currentRating && (
              ...
              <span style={{ fontSize: "10px", color: "#444", marginLeft: "auto" }}>{(idx % filtered.length) + 1} / {filtered.length}</span>
            </div>
            ...
            {!revealed && (
              <>
                <textarea
                  ...
                  rows={4}
                  style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", background: "#0d0d0d", border: "1px solid #222", color: "#e2e8f0", fontSize: "12px", outline: "none", fontFamily: "inherit", resize: "vertical", lineHeight: "1.6" }}
                />
                <div style={{ display: "flex", gap: "8px", marginTop: "10px" }}>
                  <button onClick={handleCheck}
                    style={{ flex: 1, padding: "12px", borderRadius: "8px", background: userAnswer.trim() ? "#e63946" : "#111", border: userAnswer.trim() ? "none" : "1px dashed #333", color: userAnswer.trim() ? "#fff" : "#555", fontSize: "12px", fontWeight: "700", cursor: "pointer", fontFamily: "inherit", letterSpacing: "1px" }}>
            ...
            {revealed && (
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                ...
                {/* User's answer */}
                {userAnswer.trim() && (
                  <div style={{ background: "#0d0d0d", borderRadius: "8px", padding: "14px", border: "1px solid #333" }}>
                    <div style={{ fontSize: "10px", letterSpacing: "2px", color: "#888", marginBottom: "8px" }}>YOUR ANSWER</div>
                ...
                {/* Ideal answer */}
                <div style={{ background: "#0d0d0d", borderRadius: "8px", padding: "14px", border: "1px solid #e6394630" }}>
                  <div style={{ fontSize: "10px", letterSpacing: "2px", color: "#e63946", marginBottom: "8px" }}>IDEAL ANSWER</div>
                  <pre style={{ margin: 0, fontSize: "12px", color: "#888", lineHeight: "1.7", whiteSpace: "pre-wrap", fontFamily: "inherit" }}>
                    {question.a}
                  </pre>
                </div>
              </div>
                ...
                {RATINGS.map(r => {
                  ...
                  return (
                    <button key={r.key} onClick={() => applyRating(r.key, question.id)}
                      style={{ flex: 1, padding: "10px 8px", borderRadius: "6px", border: `1px solid ${active ? r.border : "#222"}`, background: active ? r.bg : "transparent", color: active ? r.color : "#444", fontSize: "11px", cursor: "pointer", fontFamily: "inherit", fontWeight: active ? "700" : "400", transition: "all 0.2s", minWidth: "90px" }}>
                      {r.label}
                    </button>
                  );
                })}
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={prev} style={{ padding: "10px 16px", borderRadius: "6px", border: "1px solid #333", background: "transparent", color: "#555", fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>← Prev</button>
            <button onClick={shuffle} style={{ flex: 1, padding: "10px", borderRadius: "6px", border: "1px solid #333", background: "transparent", color: "#888", fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>🔀 Shuffle</button>
            <button onClick={next} style={{ padding: "10px 16px", borderRadius: "6px", background: "#e63946", border: "none", color: "#fff", fontSize: "12px", fontWeight: "800", cursor: "pointer", fontFamily: "inherit" }}>Next →</button>
          </div>
        </>
      )}
    </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "20px" }}>
        {CHEAT_SHEETS.map(s => (
          <button key={s.id} onClick={() => setActiveSheet(s.id)}
            style={{ padding: "8px 14px", borderRadius: "6px", border: `1px solid ${activeSheet === s.id ? "#e63946" : "#222"}`, background: activeSheet === s.id ? "rgba(230,57,70,0.1)" : "transparent", color: activeSheet === s.id ? "#e63946" : "#555", fontSize: "12px", cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" }}>
            {s.emoji} {s.title}
          </button>
        ))}
      </div>
      {sheet.sections.map(section => {
        ...
        return (
          <div key={section.title} style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: "8px", marginBottom: "8px", overflow: "hidden" }}>
            <div onClick={() => toggleSection(section.title)}
              style={{ padding: "12px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", background: isOpen ? "rgba(230,57,70,0.04)" : "transparent" }}>
            ...
            {isOpen && (
              <div style={{ padding: "0 16px 16px" }}>
                <pre style={{ margin: 0, fontSize: "11px", color: "#888", lineHeight: "1.8", whiteSpace: "pre-wrap", fontFamily: "'JetBrains Mono', 'Fira Code', monospace", background: "#0d0d0d", padding: "12px", borderRadius: "6px", overflowX: "auto" }}>
                  {section.content}
                </pre>
              </div>
            )}
...
export default function PrepTab() {
  ...
  return (
    <div style={{ paddingBottom: "40px" }}>
      <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#e63946", marginBottom: "16px", textAlign: "center" }}>▶ PREP.exe</div>

      <div style={{ display: "flex", gap: "6px", marginBottom: "20px", background: "#0d0d0d", borderRadius: "8px", padding: "4px", border: "1px solid #1a1a1a" }}>
        {[{ id: "mock", label: "🎤 Mock Interview" }, { id: "sheets", label: "📋 Cheat Sheets" }].map(m => (
          <button key={m.id} onClick={() => setMode(m.id)}
            style={{ flex: 1, padding: "8px", borderRadius: "6px", border: "none", background: mode === m.id ? "rgba(230,57,70,0.12)" : "transparent", color: mode === m.id ? "#e63946" : "#555", fontSize: "12px", fontWeight: mode === m.id ? "800" : "400", cursor: "pointer", fontFamily: "inherit", borderBottom: mode === m.id ? "2px solid #e63946" : "2px solid transparent", transition: "all 0.2s" }}>
            {m.label}
          </button>
        ))}
      </div>
